...
/// A screen widget for verifying OTP (One-Time Password).
/// This screen is used for verifying the OTP code sent to the user's email.
class VerifyOtpScreen extends StatefulHookConsumerWidget {
  final bool isSignUp;
  final String email;
  const VerifyOtpScreen(
      {super.key, required this.isSignUp, required this.email});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => VerifyOtpScreenState();
}

/// The state of the [VerifyOtpScreen] widget.
class VerifyOtpScreenState extends ConsumerState<VerifyOtpScreen> {
  ...
  ///
  /// This method starts a timer that counts down from the initial timer duration.
  /// It updates the [_resendTimer] value every second until it reaches 0.
  /// When the timer reaches 0, it cancels the timer.
